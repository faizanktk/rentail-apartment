...
<div class="main-wrapper">
	...
	<div class="content-title-inner">
		<div class="container">	
			...
			?>
        	<input type="hidden" value="<?php echo rtrim($address , ', ');?>" name="address" id="address"  />	
			...
			<ul class="nav nav-tabs nav-tabs-items-2" role="tablist" style="background: transparent; text-align:center;">
				<li class="nav nav-tabs nav-tabs-items-2">
			...
			<div class="tab-content listing-detail">
				<div class="tab-pane active" id="property-description" role="tabpanel" aria-expanded="true">
					<div class="gallery">
                        <?php 
                            $image=  base_url()."assets/img/tmp/house.png";
                        ...
                        ?>
                            <div class="gallery-item" style="background-image: url('<?php echo $image;?>');">
                                <?php if($property_detail['featured']=='yes'){?>
				...
				<div class="tab-pane" id="property-amenities" role="tabpanel" aria-expanded="true">
					<!-- <div id="loc_map" class="leaflet-container leaflet-fade-anim" >  -->
...
<div class="comment-create" >
	...
	<form method="post" name="comments" id="comments">
        ...
        <input type="hidden" name="comment_from_name"  class="form-control" value="<?php echo $fullname;?>">
       	<input type="hidden" name="comment_from_email"  class="form-control" value="<?php echo $email;?>">
         <input type="hidden"  name="comment_propertyemail" class="form-control"  value="<?php echo $property_detail['useremail'];?>">
        
         <i id="comment-circle" class="fa fa-circle-o-notch fa-spin pull-right" style="font-size:24px; color:#0AA998;line-height: 40px; display:none;"></i>
...
<?php if(!($this->session->userdata('loginid'))){?>
   <div class="widget widget-background-white">
    ...
    <form method="post" action="" id="propertyemailcontact">
   		 <div id="adam1" class="alert alert-success alert-dismissible" style="display:none">
              <a  href="#" class="close" onclick="hide('adam1')" aria-label="close">&times;</a>
              <span id="success">Message send successfuly!</span>
            </div>
                <div id="adam2" class="alert alert-danger alert-dismissible" style="display:none">
                  <a href="#" class="close" onclick="hide('adam2')" aria-label="close">&times;</a>
        ...
        <div class="form-group-btn">
        <i id="login-circle" class="fa fa-circle-o-notch fa-spin pull-right" style="font-size:24px; color:#0AA998;line-height: 40px; display:none;"></i>
...
<?php }else{ ?>
        ...
        <form method="post" action="" id="propertycontact">
             <div id="prop-cont-succ" class="alert alert-success alert-dismissible" style="display:none">
              <a  href="javascript:void(0)" class="close" onclick="hide('prop-cont-succ')" aria-label="close">&times;</a>
              <span id="success">Message send successfuly!</span>
            </div>
            <div id="prop-cont-dang" class="alert alert-danger alert-dismissible" style="display:none">
              <a   href="javascript:void(0)" class="close" onclick="hide('prop-cont-dang')" aria-label="close">&times;</a>
              <span id="danger">Error while sending message!</span>
            </div>
            ...
            <input type="hidden" name="from_name" id="username" class="form-control" value="<?php echo $fullname;?>">
            <input type="hidden" name="from_email" id="from_email" class="form-control" value="<?php echo $email;?>">
            <input type="hidden" class="form-control" id="propertyemail" name="propertyemail" value="<?php echo $property_detail['useremail'];?>">
            
            <div class="form-group-btn">
            <i id="islogin-circle" class="fa fa-circle-o-notch fa-spin pull-right" style="font-size:24px; color:#0AA998;line-height: 40px; display:none;"></i>
            	<button type="submit" class="btn btn-primary btn-block">Send Message</button>
       		</div><!-- /.form-group-btn --> 
...
var address= $('#address').val();
 ...
 $.getJSON( {
    ...
    success : function( data, textStatus ) {
		...
		function_ajax(data,textStatus);
		
    }
	} );
function function_ajax(res,textStatus)
{  

console.log(res.status);
	if(res.status=='OK')
	{
		latit = res.results[0].geometry.location.lat;
		longit = res.results[0].geometry.location.lng;
	}
	else
	{
		 latit ='40.761077';
	...
	if ($('#loc_map').length) {
		...
		//map.setZoom(16);
		//map.panTo([latit, longit]);// to center map at greenwich observatory (in theory)
